...
# Using Ansible to automate app deployment on {[provider]}-provided infrastructure
{: #ansible}
...
**How do Ansible and {[provider]} work together?** </br>

Ansible and {[provider]} are complimentary solutions, each addressing a key area of application and environment management. {[provider]} provides lifecycle management of infrastructure whereas Ansible helps you to provision and configure apps. 
...
## Running Ansible operations on {[provider]}-deployed infrastructure
{: #run_ansible}
...
Before you begin: 
- [Complete the {[provider]} getting started tutorial](/docs/terraform?topic=terraform-getting-started#getting-started) to install {[provider]}, configure the {{site.data.keyword.cloud_notm}} Provider plug-in and provision a virtual server in {{site.data.keyword.cloud_notm}}. 
...
## Creating a {[provider]} infrastructure inventory for Ansible
{: #create_inventory}

For Ansible to provision apps on {[provider]}-deployed infrastructure, details about the infrastructure components must be imported into Ansible. 
{: shortdesc}

**How does it work?** </br>
Ansible executes code against multiple hosts at the same time. To retrieve the host's private IP address, Ansible uses an inventory list that includes information for how to connect to each virtual server. By default, this inventory file is named [`hosts`](#specify_hosts) and is stored in the Ansible project directory. You can also use the {[provider]} inventory script to [import the infrastructure information from your {[provider]} `terraform.tfstate` file](#import_from_terraform). 
...
Specifying the virtual servers in a `hosts` file is a good way to get started with Ansible and in cases where your infrastructure does not change. The `hosts` file is a static file and must be changed every time that you add or remove a virtual server. If your infrastructure changes often or if you use {[provider]} to provision your infrastructure, you can [import infrastructure information directly from {[provider]}](#import_from_terraform). 
...
### Importing infrastructure information from {[provider]}
{: #import_from_terraform}

Use the {[provider]} inventory script to import infrastructure information from your {[provider]} `terraform.tfstate` file directly. 
{: shortdesc}

Looking to group your {[provider]} infrastructure resources for Ansible so that you can execute code against a group of hosts rather than targeting individual machines? Make sure to create your virtual servers with a group attribute. 
{: tip}

The following image shows the components of {[provider]} and Ansible. Ansible uses the inventory process that is defined in the `terraform_inv.py` script to read the state of the provisioned infrastructure from the `terraform.tfstate` file. To find the `terraform.tfstate` file, you must specify the location of the file in the `terraform_inv.ini` file. This integration point between the two provisioning tools allows dynamically allocated IP addresses, host names and infrastructure details to be passed to Ansible.

<img src="../images/terraform_ansible_dynamic_inventory.png" alt="Importing {[provider]} provided infrastructure information with Ansible dynamic inventory" width="600" style="width: 600px; border-style: none"/>

To import the {[provider]} infrastructure inventory into Ansible: 
...
2. Download the {[provider]} inventory package to a temporary folder on your local machine.  
   ```
...
4. Prepare your Ansible project folder for the {[provider]} inventory script. 
...
5. Specify the location of the `terraform.tfstate` file. The `terraform.tfstate` file defines the deployed infrastructure that you want to target with Ansible. By specifying the location of the file in a `terraform_inv.ini` file and storing it in the same directory as the {[provider]} inventory script, Ansible can import the {[provider]} infrastructure information directly.
   ...
   2. Add the path to the `terraform.tfstate` file that holds your {[provider]} infrastructure information. To test the script, you can point to one of the test files in the `tr_test_files` folder that you added earlier as shown in the following command.   
      ```
      [TFSTATE] 
...
6. Verify that the {[provider]} inventory script can import the {[provider]} infrastructure that is defined in the `terraform.tfstate` file. 
   1. Navigate to your Ansible project parent directory.  
   2. Import the {[provider]} infrastructure information from the `terraform.tfstate` file to create the Ansible infrastructure inventory. 
      ```
...
7. Verify that the Ansible inventory list updates when you add infrastructure resources. 
   1. In your {[provider]} project directory, create a file that is called `sample2.tf`. 
      ```
   ...
   4. Wait for the provisioning to complete and list your {[provider]} infrastructure resources.  
      ```
      terraform show
      ```
      {: pre}
