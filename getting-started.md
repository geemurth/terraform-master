...
{[provider]} is an Open Source software that enables predictable and consistent provisioning of {{site.data.keyword.cloud_notm}} platform, classic infrastructure, and VPC infrastructure resources by using a high-level scripting language. You can use {[provider]} to automate your {{site.data.keyword.cloud_notm}} resource provisioning, rapidly build complex, multi-tier cloud environments, and enable Infrastructure as Code (IaC).  
{: shortdesc}

Looking for a managed {[provider]} solution? Try out [{{site.data.keyword.bplong_notm}}](/docs/schematics?topic=schematics-getting-started). With {{site.data.keyword.bpshort}}, you can use the {[provider]} scripting language that you are familiar with, but you don't have to worry about setting up and maintaining the {[provider]} CLI and {{site.data.keyword.cloud_notm}} Provider plug-in. {{site.data.keyword.bpshort}} also provides pre-defined {[provider]} templates that you can easily install from the {{site.data.keyword.cloud_notm}} catalog.
{: tip}

**How does it work?**</br>
Let's say you want to spin up multiple copies of your service that uses a cluster of virtual servers, a load balancer, and a database server. You could learn how to create each resource, review the API or the commands that you need, and write a bash script to spin up these components. But it's easier, faster, and more orderly to specify the type of resource that you want and let {[provider]} do it all for you. 

The {[provider]} configuration files describe the resources that you need and how you want to configure them. Based on your configuration, {[provider]} creates an execution plan and describes the actions that need to be executed to get to the required state. You can review the execution plan, change it, or simply execute the plan. When you change your configuration, {[provider]} can determine what changed and create incremental execution plans that you can apply to your existing {{site.data.keyword.cloud_notm}} resources. 
...
Sounds great? Get started by installing the {[provider]} CLI and the {{site.data.keyword.cloud_notm}} Provider plug-in. Then, configure the {{site.data.keyword.cloud_notm}} resources that you want and watch {[provider]} spin them up. 

## Installing the {[provider]} CLI and the IBM Cloud Provider plug-in
{: #install}

To use {[provider]} to manage {{site.data.keyword.cloud_notm}} resources, you must install the {[provider]} CLI and the {{site.data.keyword.cloud_notm}} Provider plug-in for {[provider]}. 
{: shortdesc}

To support a multi-cloud approach, {[provider]} works with multiple cloud providers. A cloud provider is responsible for understanding the resources that you provision, their API, and the methods to expose these resources in the cloud. The {{site.data.keyword.cloud_notm}} Provider plug-in is aware of the {{site.data.keyword.cloud_notm}} resources that are supported and how you can configure and provision them. 

1. Install {[provider]} on your local machine. 
   ...
   2. [Download the {[provider]} CLI version 0.12.x to your local machine ![External link icon](../icons/launch-glyph.svg "External link icon")](https://releases.hashicorp.com/terraform/). 
...
To find a full list of {{site.data.keyword.cloud_notm}} resources that you can provision with the {{site.data.keyword.cloud_notm}} Provider plug-in, see the [Index of {[provider]} resources and data sources](/docs/terraform?topic=terraform-index-of-terraform-resources-and-data-sources).

1. Create a folder on your local machine for your first {[provider]} project and navigate into the folder. This folder is used to store all configuration files and variable definitions. 
   ```
...
6. Create a local {[provider]} variables file that is named `terraform.tfvars` to store your {{site.data.keyword.cloud_notm}} classic infrastructure credentials and the {{site.data.keyword.cloud_notm}} API key. Make sure to save this file in the folder that you created for your first {[provider]} project. Variables that are defined in the `terraform.tfvars` file are automatically loaded by {[provider]} when the {[provider]} CLI is initialized and you can reference them in every {[provider]} configuration file that you use. 

   Because the `terraform.tfvars` file contains confidential information, do not push this file to your version control system where you store the {[provider]} configuration files of the resources that you want to provision. This file is meant to be on your local system only. 
   {: important}
...
7. Create a {[provider]} provider configuration file that is named `provider.tf`. Use this file to configure the {{site.data.keyword.cloud_notm}} Provider plug-in with the credentials from your `terraform.tfvars` file so that the plug-in can access and provision {{site.data.keyword.cloud_notm}} resources. To reference a variable from the `terraform.tfvars` file, use the syntax `var.<variable_name>`. 
   ```
   ...
   <td>Enter <strong>1</strong> to configure the {{site.data.keyword.cloud_notm}} provider plug-in to provision VPC on Classic infrastructure resources. This value is used for all VPC resources that you specify in your {[provider]} configuration files. You can remove this parameter if you want to provision classic infrastructure resources only. </td>
   </tr>
   <tr>
...
Use {[provider]} to create a virtual server instance in a VPC, and set up networking for your VPC in your {{site.data.keyword.cloud_notm}} account. 
{: shortdesc}
...
Before you begin: 
- [Install the {[provider]} CLI and the {{site.data.keyword.cloud_notm}} Provider plug-in for {[provider]}](#install). 
...
- [Retrieve your {{site.data.keyword.cloud_notm}} credentials, upload an SSH key, and configure the {[provider]} provider plug-in](/docs/terraform?topic=terraform-getting-started#cloud_provider_config). 
...
2. In the same directory where you stored the `terraform.tfvars` and `provider.tf` files, create a {[provider]} configuration file and name it `vpc.tf`. The configuration file includes the following definition blocks: 
   ...
   <td>Enter a name that you want to append to the name of all VPC infrastructure resources that you create with this configuration file. </td>
     ...
     <tr>
       ...
       <td>Enter the name of the operating system that you want to install on your VPC virtual server instance. You use this {[provider]} resource to retrieve the ID of the operating system when you specify the VPC virtual server instance. For supported image names, run <code>ibmcloud is images</code>. </td>
     </tr>
     <tr>
       <td><code>data.ibm_is_ssh_key.name</code></td>
       <td>Enter the name of the SSH key that you uploaded to your {{site.data.keyword.cloud_notm}} account. You use this {[provider]} resource to retrieve the ID of your SSH key when you specify the VPC virtual server instance. </td>
     </tr>
     <tr>
...
3. Initialize {[provider]}. 
   ...
   * provider.ibm: version = "~> 0.11"
   ...
   If you ever set or change modules or backend configuration for {[provider]}, rerun this command to reinitialize your working directory. If you forget, other commands detect it and remind you to do so if necessary.
   ```
   {: screen}
   
4. Generate a {[provider]} execution plan. When you execute this command, {[provider]} validates the syntax of your configuration file and resource definitions against the specifications that are provided by the {{site.data.keyword.cloud_notm}} Provider plug-in. 
   ```
   terraform plan
   ```
   {: pre}
...
Create your a classic infrastructure virtual service instance with {[provider]}. Similar to the VPC virtual server instance that you provisioned earlier, you create another configuration file with the specification for your classic infrastructure virtual server instance. 
{: shortdesc}
...
4. Review the execution plan to verify the type of resource that is planned to be provisioned by {[provider]}.
...
**What's next?** </br>
[Explore other {{site.data.keyword.cloud_notm}} resources](/docs/terraform?topic=terraform-setup_cli#configure_provider) that you can provision with {[provider]}. 
