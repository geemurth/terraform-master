...
keywords: {[provider]}, ansible, wordpress, automate, automation, iaas, single site, single zone
...
# Tutorial: Deploying Wordpress on IBM Cloud classic infrastructure with {[provider]} and Ansible
{: #deploy_wordpress}
{: toc-content-type="tutorial"}
{: toc-services="terraform, virtual-servers"}
{: toc-completion-time="1h"}

Use this tutorial to automate the provisioning of classic infrastructure resources in {{site.data.keyword.Bluemix_notm}} by using {[provider]} and the deployment of Wordpress on those resources with Ansible.  
{: shortdesc}

[Ansible](https://docs.ansible.com) and {[provider]} are complimentary solutions, each address a key area of app and environment management. {[provider]} provides lifecycle management of infrastructure whereas Ansible helps you to provision and configure apps. This tutorial shows how you provision {{site.data.keyword.Bluemix_notm}} classic infrastructure with {[provider]} and then use Ansible to deploy Wordpress on Apache web servers and MariaDB, on your {[provider]}-deployed infrastructure resources. {[provider]} and Ansible are loosely integrated through the sharing of inventory information.
...
<img src="../images/wordpress_infrastructure.png" alt="Infrastructure and app components to deploy Wordpress on {{site.data.keyword.Bluemix_notm}} with {[provider]} and Ansible" width="800" style="width: 800px; border-style: none"/>
...
{[provider]} classic infrastructure components are provisioned by using {[provider]} configuration files whereas Ansible uses playbooks to automate the deployment of software components. 
...
In this tutorial, you use {[provider]} to deploy {{site.data.keyword.Bluemix_notm}} classic infrastructure components that you use to set up a Wordpress sample app by using Ansible. In particular, you will:

- Set up your environment and all the software that you need for your sample Wordpress app, such as {[provider]}, {{site.data.keyword.Bluemix_notm}} Provider plug-in, and Ansible.
- Provision {{site.data.keyword.Bluemix_notm}} classic infrastructure components for your Wordpress sample app by using {[provider]}.
- Import classic infrastructure resource information from {[provider]} to Ansible. 
...
This tutorial is intended for network administrators and software developers who want to learn how to use {[provider]} and Ansible to automate classic infrastructure and app deployment in {{site.data.keyword.Bluemix_notm}}.
...
To use {[provider]} to provision {{site.data.keyword.Bluemix_notm}} classic infrastructure resources, you must install {[provider]} and the {{site.data.keyword.Bluemix_notm}} Provider plug-in for {[provider]}. The {{site.data.keyword.Bluemix_notm}} Provider plug-in is aware of all the {{site.data.keyword.Bluemix_notm}} resources that you can provision with {[provider]}, including the API and the methods to expose these resources in the cloud.
{: shortdesc}

1. Download the {{site.data.keyword.Bluemix_notm}} {[provider]} Provider project. 
   ```
...
2. Install {[provider]} on your local machine. 
   ...
   2. [Download the {[provider]} CLI version 0.11.x to your local machine ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://releases.hashicorp.com/terraform/). 
      ...
      The {{site.data.keyword.Bluemix_notm}} Provider plug-in is not yet verified to work with {[provider]} version 0.12.x. To use the {{site.data.keyword.Bluemix_notm}} Provider plug-in, install {[provider]} version 0.11.x. 
      {: important}
...
3. Install the {{site.data.keyword.Bluemix_notm}} Provider plug-in for {[provider]}. 
   ...
   2. Create a hidden folder for your plug-in. The {{site.data.keyword.Bluemix_notm}} Provider plug-in is used only by the {[provider]} CLI and is not meant to be accessed by the user.  
      ```
...
6. Copy the {[provider]} configuration files to create your Wordpress infrastructure from the {{site.data.keyword.Bluemix_notm}} {[provider]} Provider package to your {[provider]} project directory. 
   ```
...
7. Configure the {{site.data.keyword.Bluemix_notm}} Provider plug-in. 
   1. Navigate into your {[provider]} project directory. 
      ```
...
With your {[provider]} project directory set-up, you can continue to set up your [Ansible work environment](#setup_ansible_single_site_wordpress). 
...
2. Copy the Ansible playbooks and configuration files to create your Wordpress app from the `examples` folder of the {{site.data.keyword.Bluemix_notm}} {[provider]} Provider package to your Ansible project directory. 
   ```
...
Great! Now that you completed the setup of {[provider]} and Ansible, you can start [provisioning the Wordpress infrastructure](#provision_terraform_infrastructure_single_site_wordpress) in {{site.data.keyword.Bluemix_notm}} by using {[provider]}. 
   
## Lesson 3: Provisioning the Wordpress infrastructure with {[provider]}
...
1. In your {[provider]} project directory, update the `variables.tf` with values for your target `datacenter`, `ssh_label` and `ssh_key` for your environment.

2. Deploy the classic infrastructure. 
   1. Instruct {[provider]} to deploy the classic infrastructure. {[provider]} parses the configuration files, creates an execution plan, and lists a summary of the resources that must be created. 
      ```
      terraform apply
      ```
      {: pre} 
      ...
      Do you want to perform these actions?
        {[provider]} will perform the actions described.
...
5. Verify that you can access one of the classic virtual server instances behind your {{site.data.keyword.Bluemix_notm}} classic load balancer. To test access via the classic load balancer, the {[provider]} sample configuration deploys a sample Apache web server on the classic virtual servers by using CloudInit. This Apache web server is accessible by using any of the public IP addresses for your load balancer that were returned in the previous step.
   ```
   curl <lb_ip_address>
   ```
   {: pre}
...
Use the {{site.data.keyword.Bluemix_notm}} {[provider]} inventory script to import classic infrastructure information from your {[provider]} `terraform.tfstate` file into Ansible. 
{: shortdesc}
...
2. Copy the `terraform_inv.py` and the `terraform_inv.ini` files from the {{site.data.keyword.Bluemix_notm}} {[provider]} provider `examples/ansible/ibm_ansible_dyn_inv` folder into the `inventory` directory that you created. 
   ```
...
4. Specify the location of the `terraform.tfstate` file that was created after you provisioned the {{site.data.keyword.Bluemix_notm}} classic infrastructure with {[provider]} in Lesson 3. The `terraform.tfstate` file is the source for information about the deployed classic infrastructure that you want to target with Ansible. By specifying the location of the file in a `terraform_inv.ini` file and storing it in the same directory as the {{site.data.keyword.Bluemix_notm}} {[provider]} inventory script, Ansible can import the {[provider]} classic infrastructure information directly.
...
6. Verify that the {{site.data.keyword.Bluemix_notm}} {[provider]} inventory script can import the {[provider]} classic infrastructure that is defined in the `terraform.tfstate` file. 
   1. Navigate to your Ansible project parent directory.  
   2. Import the {[provider]} classic infrastructure information from the `terraform.tfstate` file to create the Ansible infrastructure inventory. 
      ```
...
3. Complete the Wordpress setup dialog by using Ansible and the Wordpress CLI. During the setup, you provide the user details of the Wordpress admin that you want to use and configure the Wordpress database. 
   ...
   3. Run the `wp_site_setup.yaml` Ansible playbook to complete the initial setup dialog by using the Wordpress CLI. During the setup, Ansible automatically retrieves the **web_dns_name** of the load balancer by using the {[provider]} inventory integration and uses the domain name to configure the Wordpress site. 
      ```
