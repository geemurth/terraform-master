...
|`app_version`	|String|Optional|The version of the app. If you make changes to the content in the app compressed file specified by _app_path_, {[provider]} can't detect the changes. You can let {[provider]} know that your file content has changed by either changing the application compressed file name or by using this argument to indicate the version of the file.|
...
By default, the user that creates this resource is assigned the **Manager** Cloud Foundry role. {[provider]} returns an error when you assign the **Manager** or Cloud Foundry user role to yourself. User information is not persisted in the {[provider]} state file to avoid any incorrect information.
{: important}
