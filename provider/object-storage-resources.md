...
Both `archive_rule` and `expire_rule` must be managed by {[provider]} as they use the same lifecycle configuration. If user creates any of the rule outside of {[provider]} by using CLI or UI, you can see unexpected difference like removal of any of the rule or one rule overrides another. The policy cannot match as expected due to API limitations, as the lifecycle is a single API request for both archive and expire.
{: note}
