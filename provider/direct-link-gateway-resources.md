...
### Sample {[provider]} code to create direct link of dedicated type
{: #dl-gwy-sample}
...
### Sample {[provider]} code to create direct link of connect type
{: #dl-gwy-sample2}
...
The `Operational_status(Gateway operational status)` and `loa_reject_reason(LOA reject reason)` cannot be updated by using {[provider]} as the status and reason keeps changing with the different workflow actions.
{: note}
